...
- **看屏不是看镜头光轴**。笔记本摄像头在屏幕上沿，人看的是画面中心。默认把约 9° 下视当屏原点（atan(10cm/62cm)）；采满基线后改用前 60s 融合 pitch 中位数（排除过低头样本）。射线仍按光轴画。「看下 / 低头看稿」相对屏原点，不是相对摄像头。
- **转头**：Pose 肩线 yaw，近景回退脸 yaw / 头矩阵。头转了但融合视线仍朝屏中 → 「转头但仍看屏」（补偿性眼动），不是看旁边。
